...
// Mismo código pero con catch que expone el error exacto
const debugCode = `const items = $input.all();
const results = [];
for (const item of items) {
  ...
  try {
    ...
    if (!audioB64) {
      throw new Error('Sin base64. Resp: ' + JSON.stringify(audioResp).substring(0, 200));
    }
...
async function main() {
  ...
  // Asegurar que no tenga mode (all-items por defecto)
  delete n.parameters.mode;
  n.parameters.jsCode = debugCode;
  ...
  console.log(status === 200 ? 'OK — debug v2 activado. Envía un audio y dime el mensaje exacto que recibes.' : 'ERROR: ' + status);
}
