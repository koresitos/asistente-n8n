...
async function main() {
  ...
  console.log('Current mode:', n.parameters.mode);

  // Eliminar el campo mode para que use runOnceForAllItems (default)
  delete n.parameters.mode;
  n.parameters.jsCode = newCode;

  console.log('Mode after delete:', n.parameters.mode); // undefined = all items
  ...
  if (status === 200) {
    ...
    console.log('mode in result:', updated.parameters.mode);
    console.log('Uses this.helpers:', updated.parameters.jsCode.includes('this.helpers'));
    console.log('Uses $input.all():', updated.parameters.jsCode.includes('$input.all()'));
  } else {
