...
// Usa fetch nativo (Node 18+) en lugar de this.helpers.httpRequest
// para evitar el bug de 401 del task runner de n8n v2.6.3
...
for (const item of items) {
  ...
  if (audioB64) {
    try {
      ...
      transcription = '[DEBUG fetch: ' + e.message + ']';
    }
  }
...
async function main() {
  ...
  console.log(status === 200 ? 'OK — usando fetch nativo. Envía un audio.' : 'ERROR: ' + status);
}
