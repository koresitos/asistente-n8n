...
// jsCode completo de n4 reconstruido con la sección de imágenes incluida
// Usa template literals para evitar problemas de escaping de newlines
...
async function main() {
  ...
  console.log('Replacing n4 jsCode with clean version...');
  ...
  if (status === 200) {
    ...
    console.log('Has image section:', code.includes('Imágenes de referencia'));
    console.log('Has NUNCA rule:', code.includes('NUNCA digas que no puedes recibir'));
    console.log('Has template literal (no broken string):', code.includes('systemPrompt = `'));
    console.log('Has ORDER_COMPLETE:', code.includes('ORDER_COMPLETE'));
  } else {
