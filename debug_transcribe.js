...
// Mismo código pero catch expone el error exacto
const debugCode = `const config = $json.config;
...
try {
  ...
  if (!audioB64) {
    throw new Error('Sin base64 — respuesta Evolution: ' + JSON.stringify(parsed).substring(0, 300));
  }
  ...
  // DEBUG: expone el error exacto en el mensaje
  transcription = '[DEBUG: ' + e.message + ' | ' + (e.stack || '').substring(0, 150) + ']';
}
...
async function main() {
  ...
  n_transcribe.parameters.jsCode = debugCode;
  ...
  console.log(status === 200 ? 'OK — debug mode activado. Envía un audio y dime qué mensaje recibes en WhatsApp.' : 'ERROR: ' + status);
}
